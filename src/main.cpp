...
static string testRosenbrock()
{
    ...
    switch(taskNumber){
        case 1:{
            ...
            VectorXd g(2);
            MatrixXd H(2,2);
            output << objective->evalObjective(a,g,H) << endl;
            output << g <<endl;
            output << H <<endl;
            break;
        }
        case 2:
        {
            Vector2d a;
            a << -1.0,0.0;
            auto optGD = make_shared<OptimizerGD>();
            optGD->setAlphaInit(0.1);
            auto x =optGD->optimize(objective, a);
            
            output<<optGD->getIter()<<endl;
            output<<x<<endl;
            break;
        }
        case 3:
        ...
        case 4:
        {
            Vector2d a;
            a << -1.0,0.0;
            auto optNM = make_shared<OptimizerNM>();
            optNM->setGamma(0.8);
            auto x=optNM->optimize(objective, a);
            
            output<<optNM->getIter()+1<<endl;
            output<<x<<endl;
        ...
        case 5:
        {
            Vector2d a;
            a << -1.0,0.0;
            auto optBFGS = make_shared<OptimizerBFGS>();
            optBFGS->setGamma(0.8);
            auto x=optBFGS->optimize(objective, a);
            
            output<<optBFGS->getIter()+1<<endl;
            output<<x<<endl;
...
static string runIK()
{
    ...
    objective->setWeights(weights);
    objective->setTarget(target);
    
    if(taskNumber<=4){
        auto optBFGS = make_shared<OptimizerBFGS>();
        optBFGS->setIterMax(150);
        x=optBFGS->optimize(objective, x);
    }
    else if(taskNumber==5){
        auto optBFGS = make_shared<OptimizerBFGS>();
        optBFGS->setIterMax(2);
        x=optBFGS->optimize(objective, x);
        auto optNM=make_shared<OptimizerNM>();
        optNM->setIterMax(30);
        x=optNM->optimize(objective, x);
    }
